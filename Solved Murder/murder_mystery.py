...
test_text = "do you see what i see?"
test_text_two = "what in the world is going on?"
...
def get_average_sentence_length(text):
    ...
    for word in words:
        num_of_words = num_of_words + len(word)
    num_of_sentences = len(words)
    average = int(num_of_words / num_of_sentences)
    return average
...
test = TextSample(test_text, "test")
test_two = TextSample(test_text_two, "test two") 
...
def build_frequency_table(corpus):
    ...
    frequency_table = dict(zip(unique_words, count))
...
def ngram_creator(text_list):
    n = 2
    n_grams = zip(*[text_list[i:] for i in range(n)])
    return [" ".join(n_gram) for n_gram in n_grams]

print(ngram_creator(prepare_text(test_text_two)))
